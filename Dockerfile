FROM centos
RUN yum install -y php wget gcc make unzip perl iproute lsof httpd openssl-devel openssl
...
RUN wget https://jaist.dl.sourceforge.net/project/nagios/nrpe-3.x/nrpe-3.2.1.tar.gz && tar xfvz nrpe-3.2.1.tar.gz && cd nrpe-3.2.1 && ./configure && make all && make install-plugin && make install-daemon && make install-config
